...
const Login: React.FC<LoginProps> = () => {
  ...
  const validators = {
    ...
    firstName: (name: string) =>
      !name?.trim() ? "First name is required" : "",
  };
  ...
  return (
    <Container tailwindClasses="flex-row w-full min-h-[calc(100vh-50px)] p-5 justify-center items-center">
      <Container
        tailwindClasses={`flex-col justify-center items-center w-full md:w-1/3 rounded-xl p-4 bg-${themeWhite}`}
      >
        <Text
        ...
        <Button
          onClick={toggleMode}
          tailwindClasses={`text-sm underline text-${themeBlack}`}
        >
          {isSignup
